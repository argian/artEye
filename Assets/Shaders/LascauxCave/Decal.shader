﻿Shader "Lascaux/Decal"{
	//show values to edit in inspector
	Properties{
		[HDR] _Color("Tint", Color) = (0, 0, 0, 1)
		_MainTex("Texture", 2D) = "white" {}
		ObjectRot("ObjectRot", vector) = (1, 0, 0, 1)
	}
...
float3 getProjectedObjectPos(float2 screenPos, float3 worldRay)
{
	...
	//apply dir of decals:
	objectPos = Rotate3DMatrix(objectPos, ObjectRot);
	...
	fixed4 frag(v2f i) : SV_TARGET
	{
		...
		float2 projectedUv = getProjectedObjectPos(screenUv, i.ray).yz;
		//read the texture color at the uv coordinate
		fixed4 col = tex2D(_MainTex, projectedUv);
